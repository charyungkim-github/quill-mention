...
// Rfice :: IME 입력 지원을 위한 핸들러 import
import { IMEHandler } from "./ime-handler";
...
export class Mention extends Module<MentionOption> {
  ...
  // Rfice :: IME 핸들러에서 접근 가능하도록 public으로 변경
  ...
  constructor(quill: Quill, options?: Partial<MentionOption>) {
    ...
    // Rfice :: IME 핸들러 초기화
    this.imeHandler = new IMEHandler(this, quill);
  }
  ...
  upHandler() {
    if (this.isOpen && !this.existingSourceExecutionToken) {
      // Rfice :: IME composition 중 화살표 키 사용 시 compositionend 건너뛰기
      this.imeHandler.setSkipNextCompositionEnd();
      this.prevItem();
      return false;
    }
    return true;
  ...
  insertItem(
    ...
    if (!programmaticInsert) {
      ...
      const actualCursorPos = currentRange ? currentRange.index : this.cursorPos;
      const deleteLength = actualCursorPos - this.mentionCharPos;

      this.quill.deleteText(
        this.mentionCharPos,
        deleteLength, // Rfice :: 항상 현재 실제 커서 위치를 사용 (composition 확정 후에도 정확한 삭제)
        Quill.sources.USER
      );
    } else {
  ...
  onTextChange(delta: Delta, oldContent: Delta, source: EmitterSource) {
    if (source === "user") {
      if (!this.imeHandler.isComposingNow()) { // Rfice :: composition 중에는 onTextChange에서 검색하지 않음 (IMEHandler의 compositionupdate에서 처리)
  ...
  // Rfice :: IMEHandler가 필요로 하는 helper 메서드들
  getMaxChars(): number {
    return this.options.maxChars!;
  }

  getMentionDenotationChars(): string[] {
    return this.options.mentionDenotationChars!;
  }

  getIsolateCharacter(): boolean {
    return this.options.isolateCharacter!;
  }

  getAllowInlineMentionChar(): boolean {
    return this.options.allowInlineMentionChar!;
  }
  ...
  setMentionCharPos(pos: number): void {
    this.mentionCharPos = pos;
  }

  performSearch(searchTerm: string, mentionChar: string): void {
    if (this.existingSourceExecutionToken) {
      this.existingSourceExecutionToken.abandoned = true;
    }
